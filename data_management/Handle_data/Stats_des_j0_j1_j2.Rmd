...
```
Rien que 6 patients parmi les 1109 sont atteints du SIDA
...
```{r}
boxplot(df$bilan_entre_sortie_j0 ~ df$dialysis_patient_J0, outline = FALSE)

kruskal.test(df$bilan_entre_sortie_j0~ df$dialysis_patient_J0)
...
boxplot(df1$bilan_entre_sortie_j1 ~ df1$dialysis_patient_J1, outline = FALSE)
kruskal.test(df1$bilan_entre_sortie_j1~ df1$dialysis_patient_J1)
```
...
```{r}
# Temprérature à J2
freq(df2$indicatrice_temperature_j2, valid = FALSE, total = TRUE, sort = "dec")
```
...
```{r}
#index cardiaque j2
summary(df2$`index_cardiac_min_J2(L/min/m2)`)
summary(df2$`index_cardiac_moyen_J2(L/min/m2)`)
```

```{r}
#thermodilution à j2
summary(df2$`thermodilutionCO_max_J2(L/min)`)
summary(df2$`thermodilutionCO_min_J2(L/min)`)
```
...
```{r}
# PaO2_FiO2 à j2
summary(df2$PaO2_FiO2_max_J2 )
summary(df2$PaO2_FiO2_min_J2 )
...
```{r}
boxplot(df2$bilan_entre_sortie_j2 ~ df2$dialysis_patient_J2, outline = FALSE)
kruskal.test(df2$bilan_entre_sortie_j2~ df2$dialysis_patient_J2)
```
...
```{r}
# deces à J2 vs bilan j2          
boxplot(df2$bilan_entre_sortie_j2 ~ df2$indicatrice_deces_j2, outline= FALSE)
kruskal.test(df2$bilan_entre_sortie_j2~ df2$indicatrice_deces_j2)
```
...
```{r}
# Hyperthermie  vs bilan j2         
boxplot(df2$bilan_entre_sortie_j2 ~ df2$indicatrice_temperature_j2, outline= FALSE)
kruskal.test(df2$bilan_entre_sortie_j2~ df2$indicatrice_temperature_j2)
```
...
```{r}
# Glycémie  vs bilan j2         
boxplot(df2$bilan_entre_sortie_j2~ df2$indicatrice_glycemie_j2, outline = FALSE)
kruskal.test(df2$bilan_entre_sortie_j2~ df2$indicatrice_glycemie_j2)
```
...
```{r warning=FALSE}
#bilan j2 vs age
ggplot(df2) + geom_point(aes(x = anchor_age, y = bilan_entre_sortie_j2))
cor.test(df2$anchor_age, df2$bilan_entre_sortie_j2,method = "spearman",use = "complete.obs" )
```
Il ne semble pas avoir de corrélation entre l'age et le bilan entrée/sortie à J1 graphiquement.
...
```{r warning=FALSE}
#lactate_j2 vs bilan j2
ggplot(df2) + geom_point(aes(x = `lactate_J2(mmol/L)`, y = bilan_entre_sortie_j2))
cor.test(df2$`lactate_J2(mmol/L)`, df2$bilan_entre_sortie_j2,method = "spearman",use = "complete.obs" )
```
...
cor.test(df2$phlabevents_J2, df2$bilan_entre_sortie_j2,method = "spearman",use = "complete.obs" )
```
...
cor.test(df2$`albumin_J2(g/dL)`, df2$bilan_entre_sortie_j2,method = "spearman",use = "complete.obs" )
